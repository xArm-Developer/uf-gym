...
class XArm7FlipEnv(RobotTaskEnv):
    """Flip task wih XArm7 robot.
...
class XArm7PushEnv(RobotTaskEnv):
    """Push task wih XArm7 robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = XArm7(sim, block_gripper=True, base_position=np.array([-0.5, 0.0, 0.0]), control_type=control_type)
...
class XArm7ReachEnv(RobotTaskEnv):
    """Reach task wih XArm7 robot.
...
class XArm7SlideEnv(RobotTaskEnv):
    """Slide task wih XArm7 robot.
...
class XArm7StackEnv(RobotTaskEnv):
    """Stack task wih XArm7 robot.
